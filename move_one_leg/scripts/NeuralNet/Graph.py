...
class Graph:
    ...
    def initializeGraph(self):
        ...
        self.DG.add_nodes_from([5, 6, 7, 8], ActFunc = activations.tanh, active = True, pos = (0,0),bias=1)
        self.DG.add_nodes_from([9, 10, 11,12], ActFunc = activations.tanh, active = True, pos = (0,0),bias=1)
        #set edges
        ...
        for i in range(NUMLAYER):
            for j in range(NUMNEURONS):
                pos[c]= np.array([j,i])
                pos1[c] = np.array([j+0.1,i+0.1])
                c+=1
        self.setNodeAttribute(pos,"pos")
        self.setNodeAttribute(pos1,"pos1")
    ...
    def addEdgeFromParent(self,connection):
        #print("ive added an edge",connection)
        #needs ad input (x,v)
        self.WG.add_edges_from([(connection[0],connection[1],self.DG[connection[0]][connection[1]])])
    ...
    def _printGraph(self):
        #print(nx.get_edge_attributes(self.WG,"active"))
        plt.subplot(121)
        pos = nx.get_node_attributes(self.WG, "pos")
        pos1 = nx.get_node_attributes(self.WG, "pos1")
        print(pos)
        print(pos1)
        nx.draw_networkx(self.WG, pos = pos, with_labels=True, font_weight='bold')
        nx.draw_networkx_edge_labels(self.WG, pos = pos, edge_labels=nx.get_edge_attributes(self.WG, "weight"),label_pos=0.8,rotate=False)
        #nx.draw_networkx_labels(self.WG, pos = pos1, labels = nx.get_node_attributes(self.WG, "bias") )
        plt.draw()
        plt.show()
    ...
    def randomIndi(self):
        ...
        acts= np.random.choice(a=[False, True], size=(1,self.WG.number_of_edges()),p = [0.2,0.8])
        ...
        acts = np.insert(np.zeros(NPL[0]),NPL[0],acts[0])
